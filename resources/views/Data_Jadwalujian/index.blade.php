...
<div class="row">
<div class="col-xs-12">
  <a href="{{url('Data_Jadwalujian/add') }}">
  ...
  <table id="datatbl" class="table table-striped table-bordered table-hover">
    ...
    <tbody>
      @foreach($jadwalujians as $jadwalujian)
        <tr>
          <td>{{ $jadwalujian->kode_jadwalujian }}</td>
          <td>{{ $matakuliah->kode_matakuliah }}</td>
          <td>{{ $dosen->nidn }}</td>
          <td>{{ $jadwalujian->tanggalujian }}</td> 
          <td>{{ $thajaran->kode_thajaran }}</td> 
          <td>{{ $jenisujian->kode_jenisujian }}</td>  
          <td>
            <div class="hidden-sm hidden-xs btn-group">
              <a href="{{ url('Data_Jadwalujian/edit',$jadwalujian->kode_jadwalujian )}}">
                <button class="btn btn-xs btn-info"><i class="ace-icon fa fa-pencil bigger-120"></i></button>
              </a>
              <a href="{{ url('Data_Jadwalujian/hapus',$jadwalujian->kode_jadwalujian )}}">
